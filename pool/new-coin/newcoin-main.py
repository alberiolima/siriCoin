...
import requests, time, json, threading, flask, rlp, datetime
...
class Beacon(object):
    ...
    def __init__(self, data, difficulty):
        ...
        self.number = 0
        self.son = ""
        self.blockChainVersion = ""
        self.q_average_time_diff = 0
        self.average_time_diff = 0
        if "version" in miningData:
            self.blockChainVersion = miningData["version"]
...
class BeaconChain(object):
    def __init__(self):
        ...
        self.blockReward = 1
        self.blockTime = 120 # in seconds, about 2 minutes
        self.qCalcDiff = 0
        self.dcalcDiff = 0
    ...
    def calcDifficulty(self, expectedDelay, elapsed_time, currentDiff):
        return min(max((currentDiff * expectedDelay)/max(elapsed_time, 1), currentDiff * 0.9, 1), currentDiff*1.1)
        
    def calcDifficulty2(self, expectedDelay, elapsed_time, currentDiff):
        if (elapsed_time < 1):
            return currentDiff
        new_diff = (currentDiff * expectedDelay)/elapsed_time
        if new_diff > currentDiff * 1.05:
            new_diff = currentDiff * 1.05
        elif new_diff < currentDiff * 0.95:
            new_diff = currentDiff * 0.95
        return int(new_diff)
    ...
    def addBeaconToChain(self, beacon):
        ...
        _block_version = beacon.blockChainVersion
        ...
        elapsed_mined_time = int(beacon.timestamp) - _oldtimestamp
        if ((elapsed_mined_time>0) and (elapsed_mined_time != self.blockTime)):
            self.difficulty += ((self.blockTime - elapsed_mined_time) * 1000 )
    ...
    def submitBlock(self, block, showMessage):
        ...
        if beaconValidity[0]:
            ...
            #self.addBeaconToChain2(_beacon) #Test new version
            if showMessage:
...
class Node(object):
    ...
    def initNode(self):
        ...
        try:
            self.loadDB()
            print("[initNode]","Successfully loaded node DB !")
        except:
            print("[initNode]","Error loading DB, starting from zero :/")
        print("[initNode]self.upgradeTxs()")
        self.upgradeTxs()
        len_self_txsOrder = len(self.txsOrder)
        q = 1;
        for txHash in self.txsOrder:
            print("self.state.playTransaction", q, "/", len_self_txsOrder )
            q += 1
            tx = self.transactions[txHash]
            if self.canBePlayed(tx)[0]:
                self.state.playTransaction(tx, False)
            # self.propagateTransactions([tx])
        print("[initNode]","self.saveDB()")
        self.saveDB()
        print("[initNode]","self.syncByBlock()")
        self.syncByBlock()
        print("[initNode]","self.saveDB()")
        self.saveDB()
...
#        print(toCheck)
    ...
    def syncByBlock(self):
        ...
        self_bestBlockChecked = self.bestBlockChecked
        self_getChainLength = self.getChainLength()
        if (self_bestBlockChecked == self_getChainLength-1):
            return
        print("[syncByBlock]","self.bestBlockChecked:",self_bestBlockChecked,"self.getChainLength():",self_getChainLength )
        start_time = time.time()
        q_proc = 0
        for blockNumber in range(self_bestBlockChecked,self_getChainLength):
            ...
            q_proc += 1
            time_block = (time.time() - start_time ) / q_proc
            rest_time = int((self_getChainLength - blockNumber) * time_block)
            print("[syncByBlock]", blockNumber, "/", self_getChainLength, "restTime (min): ", datetime.timedelta(seconds=rest_time))
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=1001, ssl_context=ssl_context)
